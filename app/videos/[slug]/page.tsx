...
export default async function VideosPage({ params }: { params: { slug: string } }) {
  ...
  if (!article || article.category?.slug !== "video-content") {
    notFound()
  }
    ...
    <div className="min-h-screen bg-white">
      <header className="border-b">
        <div className="container mx-auto px-4 py-4">
          <h1 className="text-2xl font-bold">KOODOS - Videos</h1>
        </div>
      </header>
      ...
      {article.status === "DRAFT" && (
        <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4">
          ...
          <p>This is a draft video article and is not visible to the public.</p>
        </div>
      )}
